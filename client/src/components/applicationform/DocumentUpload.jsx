...
const Documentupload = () => {
  ...
  // On mount, check if user has already uploaded a document (localStorage + DB check)
  useEffect(() => {
    ...
    // If already uploaded document, redirect to dashboard or next step
    if (localStorage.getItem('documentUploadCompleted') === 'true') {
    ...
    axios
      .get(`https://e-bursary-backend.onrender.com/api/get-document/${userId}`)
      .then((response) => {
        // If data found, set flag and redirect
        localStorage.setItem('documentUploadCompleted', 'true');
        navigate('/studentdashboard');
      })
      .catch((error) => {
        if (error.response && (error.response.status === 404 || error.response.status === 400)) {
          ...
          setUploadStatus("Could not confirm your progress. Please sign in again.");
          navigate('/signin');
        }
      });
  }, [navigate]);
